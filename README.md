...
- 下のツールバー：`塗る`(長押し) / `印`(長押し) / `戻す` / `進む` / `ヒント` / `直す` / `全体表示`。`直す`は間違い(赤いヒント)があるときだけ現れ、間違い周辺の5×5マスをまとめて消します。拡大・縮小は2本指ピンチで行います。
- 左上 `☰` で難易度変更・新しい問題・**中断して保存**・リセット・**中断中のゲーム一覧**。右上 `📊` で統計（総プレイ時間・プレイ数・クリア数・ヒント数）。
- **中断して保存**：今のゲームを一覧に残して新しい問題を開始します。後で `☰` の「中断中のゲーム」から **再開** できます（一覧には難易度・サイズ・進捗%・経過時間・保存日時が表示）。中断中のゲームを再開すると、それまで遊んでいたゲームも(進行中なら)自動的に一覧へ退避されるので失われません。

## 難易度
| 難易度 | サイズ | 形 | 特徴 |
|---|---|---|---|
| ふつう | 20〜30 | 長方形 | ヒント多め |
| むずかしい | 20〜35 | 長方形 | ヒント少なめ |
| 鬼（既定）| 20〜40（毎回変動）| いびつな形 | ヒント最小・重なり推論必須 |

※サイズは毎回ランダムに変動します（最低 20×20）。
